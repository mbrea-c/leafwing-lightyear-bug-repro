...
impl Plugin for ExampleServerPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(FixedUpdate, trigger_rollbacks);
    }
}

/// We need some data being networked in order to force rollbacks
#[derive(Component, Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct SomeData(pub f32);
...
/// Make changes in the server just to trigger client rollbacks
fn trigger_rollbacks(mut q_some_data: Query<&mut SomeData>) {
    for mut some_data in &mut q_some_data {
        some_data.0 += 0.032;
    }
}
